...
ViewUser::ViewUser(const http::request<http::string_body> &_req,
                   const std::shared_ptr<sql::Connection> &_conn, int _userId)
    : View(_req, _conn, _userId) {}

http::response<http::string_body> ViewUser::put() { return defaultPlug(); }
...
http::response<http::string_body> ViewUser::get() {
  ...
  int userId = -1;
  if ((js.contains("user_id") && js["user_id"].is_number_integer()) ||
      js.contains("login")) {
    std::string login;
    int id = -1;
    ...
    std::unique_ptr<sql::PreparedStatement> userStmt(conn->prepareStatement(
        "SELECT id, name, surname, sex, birthday, location, login FROM user "
        "where login = ? OR id = ?;"));
...
http::response<http::string_body> ViewUser::post() {
  ...
  if (js.contains("name") && js.contains("surname") && js.contains("sex") &&
      js.contains("location") && js.contains("birthday")) {
    std::string name = js["name"], surname = js["surname"], sex = js["sex"],
                location = js["location"], birthday = js["birthday"];
    int validateCode = validate(name, surname, sex, location, birthday);
    if (!validateCode) {
      std::unique_ptr<sql::PreparedStatement> userStmt(
          conn->prepareStatement("Update user set name = ?, surname = ?, sex = "
                                 "?, birthday = ?, location = ? where id = ?"));
...
http::response<http::string_body> ViewUser::delete_() { return defaultPlug(); }

int ViewUser::validate(const std::string &name, const std::string &surname,
                       const std::string &sex, const std::string &location,
                       const std::string &birthday) {
  if (!(name.length() > 2 && name.length() < 45)) return 1;
  if (!(surname.length() > 2 && surname.length() < 45)) return 2;
  if (sex != "male" && sex != "female") return 3;
  if (!(location.length() > 5 && location.length() < 45)) return 4;
  try {
    using namespace boost::gregorian;
    date birth(from_string(birthday));
    date now(day_clock::local_day());
    years year(16);
    if (birth.is_not_a_date() || (birth + year) > now) return 5;
  } catch (...) {
    return 5;
  }
  return 0;

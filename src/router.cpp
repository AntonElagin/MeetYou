...
IObjHandler *Chat_router::parse(std::string target) {
    ...
    std::regex r_message("/chat/(add|upd)/message\\?(body)=(\\w+)&(chatid)=(\\d+)");
    std::regex r_del_message("/chat/(del)/message\\?(id)=(\\d+)&(chatid)=(\\d+)");
    ...
    else if (std::regex_match(target.c_str(), result, r_message))
        return_handler = new MessageHandler;
    else if (std::regex_match(target.c_str(), result, r_user))

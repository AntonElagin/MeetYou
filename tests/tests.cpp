 ...
 protected:
  void SetUp() override {
    ...
    std::shared_ptr<sql::Connection> conn(
        driver->connect("tcp://127.0.0.1:3306", "root", "167839"));
    con = conn;
...
TEST_F(QuickTest, requests) {
  ...
  boost::string_view heh = kek.body();  ///может быть чат с полностью одинаковым
                                        ///списком юзеров(будут разные id)
  ASSERT_STREQ(heh.to_string().c_str(),
               "{\"message\":\"chat created\",\"status\":\"OK\"}");
}
...
TEST_F(QuickTest, history) {
  ...
  ASSERT_STREQ(heh.to_string().c_str(),
               "[\"message\",\"access denied\",[\"status\",\"OK\"]]");
}
...
TEST_F(QuickTest, members_list) {
  ...
  boost::string_view heh = kek.body();
  ASSERT_STREQ(heh.to_string().c_str(),
               "[[2,\"Cchaddie Disdel\"],[3,\"Maxine Fanthome\"],[4,\"Elijah "
               "Attwell\"],[\"status\",\"OK\"]]");
}
...
TEST_F(QuickTest, chat_view) {
  ...
  //    boost::string_view heh = kek.body();
  //    ASSERT_STREQ(heh.to_string().c_str(),
  //                 "{\"content\":[[2,\"Cchaddie Disdel\"],[3,\"Maxine
  //                 Fanthome\"],[4,\"Elijah
  //                 Attwell\"]],\"message\":\"OK\",\"status\":200}");
}
